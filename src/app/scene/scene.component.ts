...
const CanvasRenderer = require('../lib/CanvasRenderer.js');
const Projector = require('../lib/Projector.js');
...
export class SceneComponent implements OnInit {
  ...
  @HostListener('mousemove', ['$event']) handleMouseMove(event: MouseEvent) {
    ...
    let vector = new THREE.Vector3( ( (event.clientX + window.pageXOffset) / this.renderer.domElement.width ) * 2 - 1, - ( (event.clientY + window.pageYOffset) / this.renderer.domElement.height ) * 2 + 1, 0.5 )
    this.mouse3D = vector.unproject(this.camera);
  ...
  @HostListener('mouseup', ['$event']) handleMouseUp(event: MouseEvent) {
    ...
    var intersect, intersects = this.raycaster.intersectObjects( this.scene.children );
    ...
    if ( intersects.length > 0 ) {
      ...
      if ( intersect ) {
        ...
        if ( this.isShiftDown ) {
          ...
          var voxel = new THREE.Mesh( this.cube, new THREE.MeshStandardMaterial( this.colors[ this.color ] ) );
  ...
  private init() {
    ...
    this.linesMaterial = new THREE.LineBasicMaterial( 0x000000, 0.2 );
    ...
    this.brush = new THREE.Mesh( this.cube, new THREE.MeshStandardMaterial( this.colors[ this.color ], 0.4 ) );
    ...
    if ( version == "A/" ) {
      ...
      while ( i < l ) {
        ...
        if ( code.charAt( 0 ) == "1" ) {
          ...
          var voxel = new THREE.Mesh( this.cube, new THREE.MeshStandardMaterial( this.colors[ current.c ] ) );
      ...
      for ( var i = 0; i < data.length; i += 4 ) {
        ...
        var voxel = new THREE.Mesh( this.cube, new THREE.MeshStandardMaterial( this.colors[ data[ i + 3 ] ] ) );
  ...
  interact() {
    ...
    var position, intersect, intersects = this.raycaster.intersectObjects( this.scene.children );
